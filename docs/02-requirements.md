...
- **AC-06.7** Given a **network-policy-friction** finding, When its link is followed, Then the page lands at the needs-attention section under the filters and date behaviour of AC-06.5. Findings are **recomputed and reranked** using the [existing rules](01-metrics-contract.md#6-attention-rules). Once the destination response arrives, show “Findings recalculated for the selected filters.” Render only its current findings and evidence (domain shown where AC-06.9 permits), including an empty result. Do not track or highlight the original finding, pin it, display a fourth finding, or retain stale evidence as current.
...
- **AC-09.8** Given a `VIEWER`, Then no raw denied domain and no internal domain-bearing identity string appears in any response field, finding text or link. Neither role receives a public finding ID or an internal finding identity.
...
*Network-friction links* land at the filtered needs-attention section with newly computed findings. A notice confirms recalculation once the destination response arrives. The page does not match individual findings across responses. Nothing is pinned, the cap is never exceeded, and stale evidence is never presented as current.
...
- **PR-3** The `VIEWER` presentation **preserves evidence counts** while omitting raw denied domains and internal domain-bearing identity strings. Internal finding identity never leaves the rule engine; no public finding ID is needed. P0 has a `VIEWER` role and [automated privacy tests](05-testing-spec.md#3-backend-integration-and-apifunctional-tests).
